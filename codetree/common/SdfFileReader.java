...
public class SdfFileReader
{
    ...
    public static  List<Graph> readFile_subgraph(Path sdfFile)
    {
        int numOfgraph = 0;
        file = sdfFile;
        ...
        try (BufferedReader br = Files.newBufferedReader(sdfFile)) {
            Graph g;
            while ((g = next(br)) != null && numOfgraph<40000) {//入力ファイルからグラフデータを読み込む
                G.add(g);
                numOfgraph++;
            }
        } catch (Exception e) {
    ...
    public static  void readFile_double(Path sdfFile,List<Graph> Super ,List<Graph> Sub)
    {
    ...
    private static Graph next_gfu(BufferedReader sdf) throws Exception
    {
        ...
        while ((line = sdf.readLine()) != null) {
            ...
            if (!line.startsWith("#0") && line.startsWith("#")) {
                ...
                if(mol.size()==0 && graphID!=null)mol.add(graphID); 
                mol.add(line);
            }
        }
        if(count==0){
